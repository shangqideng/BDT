...
class PatchMerging_un(nn.Module):
    r""" Patch Merging Layer.
...
class Direction1(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=31,
        ...
        self.downsample2 = downsample(dim=self.embed_dim * 2, input_resolution=(self.img_size // 2, self.img_size // 2),
                                      norm_layer=norm_layer)
        self.downsample3 = downsample_un(dim=self.embed_dim * 4, input_resolution=(self.img_size // 4, self.img_size // 4),
                                      norm_layer=norm_layer)

        self.stage1 = Stage(dim=self.embed_dim, input_resolution=(self.img_size, self.img_size),
                            num_heads=num_heads[0], window_size=window_size)
        self.stage2 = Stage(dim=self.embed_dim * 2, input_resolution=(self.img_size // 2, self.img_size // 2),
                            num_heads=num_heads[1], window_size=window_size)
        self.stage3 = Stage(dim=self.embed_dim * 4, input_resolution=(self.img_size // 4, self.img_size // 4),
                            num_heads=num_heads[2], window_size=window_size)
        self.stage4 = Stage(dim=self.embed_dim * 4, input_resolution=(self.img_size // 8, self.img_size // 8),
                            num_heads=num_heads[3], window_size=window_size)
    ...
    def forward(self, x):
        ...
        stage3_out = self.downsample3(stage3_out)  # B (H/8 W/8) C
        H = H // 2

        stage4_out = self.stage4(stage3_out)
        stage4_out_shape = rearrange(stage4_out, 'B (H W) C -> B C H W', H=H)  # B x 4C x H/4 x W/4


        return stage1_out_shape, stage2_out_shape, stage3_out_shape, stage4_out_shape
...
class Direction2(nn.Module):
    def __init__(self, inp_channels=31, dim=32, num_heads=[8, 8, 8], group=8, ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False):
        ...
        self.stage3 = XCABlock(dim=dim, num_heads=num_heads[0], win=group, ffn_expansion_factor=ffn_expansion_factor,
                          bias=bias, LayerNorm_type=LayerNorm_type)
        self.up3_4 = Upsample_unc(int(dim))
        self.stage4 = XCABlock(dim=dim, num_heads=num_heads[0], win=group, ffn_expansion_factor=ffn_expansion_factor,
                               bias=bias, LayerNorm_type=LayerNorm_type)
    ...
    def forward(self, x):
        ...
        stage_output3_up = self.up3_4(stage_output3)  # B x dim/4 x 4h x 4w

        stage_output4 = self.stage4(stage_output3_up)

        return stage_output4, stage_output3, stage_output2, stage_output1
...
class Merge(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=32, num_heads1=[8, 8, 8],window_size=8, group=8, mlp_ratio=4.,dim=32,
        ...
        self.merge2 = nn.Sequential(nn.Conv2d(embed_dim * 4 + dim + dim, dim, kernel_size=3, padding=1, bias=bias),
                                    nn.Conv2d(dim, dim, kernel_size=5, padding=2, bias=bias),
                                    nn.LeakyReLU(0.2, bias))

        self.up2 = Upsample_unc(int(dim))

        self.merge3 = nn.Sequential(nn.Conv2d(embed_dim * 2 + dim + dim, dim, kernel_size=3, padding=1, bias=bias),
                                    nn.Conv2d(dim, dim, kernel_size=5, padding=2, bias=bias),)

        self.up3 = Upsample_unc(int(dim))
        self.merge4 = nn.Sequential(
            nn.Conv2d(embed_dim + dim + dim, in_chans2, kernel_size=3, padding=1, bias=bias),
            nn.Conv2d(in_chans2, in_chans2, kernel_size=5, padding=2, bias=bias), )

    def forward(self, x, y):
        D1_stage1, D1_stage2, D1_stage3, D1_stage4 = self.direction1(x)
        D2_stage4, D2_stage3, D2_stage2, D2_stage1 = self.direction2(y)

        merge1 = self.merge1(torch.cat((D1_stage4, D2_stage1), 1))
        merge2 = self.merge2(torch.cat((D1_stage3, D2_stage2, self.up1(merge1)), 1))
        merge3 = self.merge3(torch.cat((D1_stage2, D2_stage3, self.up2(merge2)), 1))
        output = self.merge4(torch.cat((D1_stage1, D2_stage4, self.up3(merge3)), 1))

        return output


if __name__ == '__main__':
    xt = torch.randn(1, 34, 128, 128).cuda()
    ms = torch.randn(1, 31, 16, 16).cuda()

    t = Merge(img_size=128, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8, 8], window_size=8, group=8,  mlp_ratio=4.,dim=32,
                 num_heads2=[8, 8, 8, 8],  ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).cuda()

    w_out = t(xt, ms)

    num_params = 0
    for param in Merge(img_size=128, patch_size=1, in_chans1=34, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8, 8], window_size=8, group=8,  mlp_ratio=4.,dim=32,
                 num_heads2=[8, 8, 8, 8],  ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False).parameters():
