...
class Bidinet(nn.Module):
    def __init__(self, args):
        ...
        self.dim = 48  # w-xca
        self.t = Merge(img_size=self.img_size, patch_size=1, in_chans1=34, in_chans2=self.in_channels, embed_dim=self.embed_dim, num_heads1=[8, 8, 8], window_size=8, group=8,  mlp_ratio=4.,dim=self.dim,
                 num_heads2=[8, 8, 8],  ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False)
        # self.t = T(img_size=self.img_size, patch_size=1, in_chans=34, embed_dim=32, depths=[2, 4], num_heads=[8, 8], window_size=8)
